...
# Usage: bash setup_cron.sh (from server/ directory)
# Set M5_ASTRO_SERVER_DIR if you want to override the default path.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DIR="${M5_ASTRO_SERVER_DIR:-$SCRIPT_DIR}"
...
if [ ! -f "$UPDATE_SCRIPT" ] || [ ! -f "$SERVE_SCRIPT" ]; then
    ...
    echo "Set M5_ASTRO_SERVER_DIR to the directory containing update_dashboard.py and serve.py."
    exit 1
fi
...
cd "$DIR" && "$VENV" "$SERVE_SCRIPT" 8081 > "$DIR/http.log" 2>&1 &
echo "Running initial update..."
cd "$DIR" && "$VENV" "$UPDATE_SCRIPT"
